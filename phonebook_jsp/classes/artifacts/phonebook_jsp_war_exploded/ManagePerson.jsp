...
<table align="center" border="1" width="70%">
    ...
    <td><table>
        <%
            ...
            {
        %><tr>
        <td align="left" width="40%"><%out.write(phone.getValue() + "\n");%>
            <input type="hidden" name="phone_id" value="<%=phone.getKey()%>"/>
            <a href="<%=request.getContextPath()%>/?action=edit_phone&id=<%=person.getId()%>">Редактировать</a>
            <a href="<%=request.getContextPath()%>/?action=delete_phone&id=<%=person.getId()%>">Удалить</a>
        <a href="<%=request.getContextPath()%>/?action=delete_phone&id=<%=person.getId()%>&phone_id=<%=phone.getKey()%>">Удалить</a>
        </td>
